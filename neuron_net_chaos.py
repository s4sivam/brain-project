# neuron_net_chaos.py
# A faster, more chaotic version of the brain.
# It evolves faster, forgets faster, and has more random ideas.
...
# --- CHAOS MODIFICATIONS ---
DECAY_INTERVAL = 200
DECAY_FACTOR = 0.980       # <-- FASTER FORGETTING (was 0.995)

PRUNE_INTERVAL = 250       # <-- FASTER EVOLUTION (was 400)
PRUNE_THRESH = 0.02
PRUNE_PROB = 0.4
SPROUT_K = 20              # <-- MORE RANDOM IDEAS (was 14)
CORR_WINDOW = 400
# --- END MODIFICATIONS ---
...
class Network:
    ...
    def prune_weak(self):
        remaining = []
        for s in self.synapses:
            if s.w < PRUNE_THRESH and random.random() < PRUNE_PROB:
                if s in s.pre.outgoing:
                    s.pre.outgoing.remove(s)
            else:
                remaining.append(s)
        self.synapses = remaining

    def compute_correlation_scores(self):
        counts = {n.idx: len(n.spike_times) for n in self.neurons}
        existing = set((s.pre.idx, s.post.idx) for s in self.synapses)
        scores = []
        for pre in self.neurons:
            for post in self.neurons:
                if pre is post: continue
                key = (pre.idx, post.idx)
                if key in existing: continue
                
                bias = 1.0
                if pre in self.inputs and post in self.hidden: bias = 2.0
                
                score = (counts.get(pre.idx, 0) * counts.get(post.idx, 0)) * bias + random.random() * 0.05
                scores.append((score, pre, post))
        scores.sort(key=lambda x: x[0], reverse=True)
        return scores

    def sprout_new(self):
        scores = self.compute_correlation_scores()
        created = 0
        for score, pre, post in scores:
            if created >= SPROUT_K: break
            if score <= 0 and random.random() > 0.02: continue
            
            delay = random.randint(1, 2)
            s = Synapse(pre, post, w=random.uniform(0.015, 0.08), delay=delay)
            pre.outgoing.append(s)
            self.synapses.append(s)
            created += 1
